...
function iniciar_sesion() {
  ...
  if (curp == "") {
    Swal.fire("Error", "CURP no puede estar vacío", "error");
    return;
  }
  ...
  if (curp != contrasenia) {
    Swal.fire("Error", "El CURP y la contraseña no coinciden", "error");
    return;
  }
  $.ajax({
    type: "post",
    ...
    success: function(response) {
      ...
      if (response != "no") {
        ...
        Swal.fire("Sesion no iniciada", "Usuario no encontrado", "warning");
      }
    },
    error: function(xhr) {
  ...
  $.ajax({
    ...
    success: function(response) {
      ...
      if (response == "OK") {
        ...
        Swal.fire("Registrar libro", "El libro no se ha registrado", "error");
      }
    },
    error: function(xhr) {
      console.log(xhr.responseText);
...
function registrar_categoria() {
  ...
  $.ajax({
    ...
    error: function(xhr) {
      console.log(xhr.responseText);
      Swal.fire("Error", "Ha ocurrido un error del sistema", "error");
    }
  });
}
...
function prestar_libro() {
  ...
  var curp_usuario = $("#usuario_prestar").val();
  // Swal.fire(curp_usuario);
  if (curp_usuario == "") {
    curp_usuario = localStorage.getItem("CurpUsuario");
  }
...
function editar_usuario(id_usuario = 0) {
  // Manda una petición AJAX de tipo JSON, es decir, espera recibir varios datos de tipo {clave: valor}
  $.ajax({
    type: "POST",
    url: "functions_class.php",
    data: {
    ...
    success: function(response) {
      // console.log(response);
      // Recibió una estructura en forma de arreglo JSON {clave: valor} Y se recorre así.
      $("#curp_registro").val(response.usuario.CurpUsuario);
      $("#usuario_registro").val(response.usuario.NombreUsuario);
      $("#grado_grupo_registro").val(response.usuario.GradoGrupo);
      $("#btn-registrar_usuario").attr(
        "onclick",
        "modificar_usuario(" + response.usuario.IdUsuario + ")"
      );
      $("#btn-registrar_usuario").text("Modificar registro");
    },
    error: function(xhr) {
      // Si algo sucede en la petición AJAX Regresa y coloca el error en la consola.
      console.log(xhr.responseText);
    }
  });
}

function modificar_usuario(id_usuario = 0) {
  var curp_registro = $("#curp_registro").val();
  if (curp_registro == "") {
    Swal.fire("Error", "CURP no puede venir vacío", "warning");
    return;
  }
  var usuario_registro = $("#usuario_registro").val();
  if (usuario_registro == "") {
    Swal.fire("Error", "Nombre de usuario no puede venir vacío", "warning");
    return;
  }
  var grado_grupo_registro = $("#grado_grupo_registro").val();
  if (grado_grupo_registro == "") {
    Swal.fire("Error", "Grado y grupo no puede venir vacío", "warning");
    return;
  }
  $.ajax({
    type: "POST",
    ...
    success: async function(response) {
      console.log(response);
      if (response == "OK") {
        await Swal.fire("Registro modificado", "Actualizando datos", "success");
        window.location.reload();
      } else {
        await Swal.fire("Error", "El registro no se modificó", "error");
      }
    }
  });
}

function eliminar_usuario(id_usuario = 0) {
  Swal.fire({
      ...
      $.ajax({
        ...
        success: async function(response) {
          if (response == "OK") {
            ...
            Swal.fire(
              "Error",
              "No se ha dado de baja al usuario, recargue la página y vuelva a intentar",
              "error"
            );
          }
        }
